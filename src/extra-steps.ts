...
export function seedUserInput(): PromptStep<{ spendingItem: string; userValue: string }, { spendingItem: string; userValue: string }> {
  return async (ctx) => {
...
// 1: Classify category
export const classifyCategory: PromptStep<{ spendingItem: string; userValue: string }, { spendingItem: string; userValue: string; category: string }> = async (ctx) => {
...
export const webScanPrices: PromptStep<
  ...
  { spendingItem: string; userValue: string; category: string; kpis: any; pricingRaw: Array<{ name: string; price: number }> }
> = async (ctx) => {
  const prompt =
  ...
  return {
    ...
    transcript: [
      ...ctx.transcript,
      { role: 'user', content: prompt },
      { role: 'assistant', content: jsonArrText }
    ],
    session
  };
};
...
export function dedupeIdeas(): PromptStep<{ rawIdeas: string[] }, { ideas: string[] }> {
  return async (ctx) => {
    const unique = Array.from(new Set(ctx.data.rawIdeas.map(i => i.toLowerCase())))
      .map(i => ctx.data.rawIdeas.find(orig => orig.toLowerCase() === i)!);
...
// 7: Filter by score
export function filterByScore(threshold = 0.65): PromptStep<any, { shortlist: Array<{ idea: string; score: number }> }> {
  return async (ctx) => {
    const shortlist = ctx.data.scored.filter((item: any) => item.score >= threshold)
...
// 8: Estimate savings
export const estimateSavings: PromptStep<any, { shortlist: Array<{ idea: string; score: number; savings: number }> }> = async (ctx) => {
  const prompt = `Based on pricing data ${JSON.stringify(ctx.data.pricingRaw)}, estimate yearly savings for each idea ${JSON.stringify(ctx.data.shortlist.map(i=>i.idea))}. Output JSON [{"idea":string,"savings":number}].`;
  const { newContent, session } = await sendPrompt(ctx, prompt);
  const jsonText = extractFirstJson(newContent);
	let savingsArr: any[];
	try {
...
export const tradeoffTable: PromptStep<any, { tradeoffsMarkdown: string }> = async (ctx) => {
  const prompt = `Create a markdown table comparing idea, score, and savings for: ${JSON.stringify(ctx.data.shortlist)}.`;
  const { newContent, session } = await sendPrompt(ctx, prompt);
  return {
...
export const generateActions: PromptStep<any, { actionPlanSteps: string[] }> = async (ctx) => {
  ...
  const { newContent, session } = await sendPrompt(ctx, prompt);
  const actions = newContent.split(/\n+/).map(l => l.replace(/^-/, '').trim()).filter(Boolean);
  return {
...
export const summaryExecutive: PromptStep<any, { summary: string }> = async (ctx) => {
  const prompt = `Write an executive summary (<250 words) highlighting the top 3 recommendations: ${JSON.stringify(ctx.data.ranked.slice(0,3))}.`;
  const { newContent, session } = await sendPrompt(ctx, prompt);
  return {
...
> = async (ctx) => {
  const prompt = `Given these sub-types: ${JSON.stringify(ctx.data.subtypes)} and KPI weights ${JSON.stringify(ctx.data.kpis)}, pick the 2 that best match the user’s priorities. Respond with a JSON array of 2 strings.`;
  const { newContent, session } = await sendPrompt(ctx, prompt);
  const arrText = extractFirstJson(newContent);
 ...
 */
export function flattenIdeas(): PromptStep<
  { rawIdeas: Record<string,string[]> },
  { rawIdeas: string[] }
> {
  return async (ctx) => {
 ...
 */
export function groupAndLimit(maxPerGroup = 2): PromptStep<
...
export const noveltyFilter: PromptStep<any, any> = async (ctx) => {
  const prompt = `From this list of options:\n\n${JSON.stringify(ctx.data.ideas, null, 2)}\n\nRemove any that are essentially the same as "${ctx.data.spendingItem}". Respond with a JSON array of the remaining unique options.`;
  const { newContent, session } = await sendPrompt(ctx, prompt);
  const arrText = extractFirstJson(newContent);
  const ideas = JSON.parse(arrText);
  return {
    ...
    transcript: [
      ...ctx.transcript,
      { role: 'user', content: prompt },
      { role: 'assistant', content: arrText }
    ],
    session
  };
};
